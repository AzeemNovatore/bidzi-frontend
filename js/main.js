...
(function ($) {
  $.fn.contentTabs = function (o) {
    ...
    return this.each(function () {
      ...
      if (
        options.checkHash &&
        tabLinks.filter("[" + options.attrib + '="' + location.hash + '"]')
          .length
      ) {
        (options.addToParent ? tabLinksParents : tabLinks).removeClass(
          options.activeClass
        );
        setTimeout(function () {
          window.scrollTo(0, 0);
      ...
      tabLinks.each(function () {
        ...
        if (
          classOwner.hasClass(options.activeClass) ||
          (options.checkHash && location.hash === href)
        ) {
      ...
      function switchTab(oldLink, newLink) {
        ...
        (options.addToParent ? newLink.data("cparent") : newLink).addClass(
          options.activeClass
        );
      ...
      if (options.autoHeight) {
        $(window).bind("resize orientationchange", function () {
          tabs
            .not(currentTab)
            .removeClass(tabHiddenClass)
            .show()
            .each(function () {
              var tab = jQuery(this),
                tabWidth = tab.css({ width: "" }).width();
              if (tabWidth) {
                tab.width(tabWidth);
              }
            })
            .hide()
            .addClass(tabHiddenClass);
      ...
      function nextTab() {
        var activeItem = (
          options.addToParent ? tabLinksParents : tabLinks
        ).filter("." + options.activeClass);
        var activeIndex = (
          options.addToParent ? tabLinksParents : tabLinks
        ).index(activeItem);
        var newLink = tabLinks.eq(
          activeIndex < tabLinks.length - 1 ? activeIndex + 1 : 0
        );
  ...
  var contentTabsEffect = {
    ...
    slide: {
      show: function (o) {
        var tabHeight = o.tab
          .show()
          .css({ width: o.tab.width() })
          .outerHeight(true);
        var tmpWrap = $('<div class="effect-div">')
          .insertBefore(o.tab)
          .append(o.tab);
        tmpWrap.css({
          width: "100%",
      ...
      hide: function (o) {
        ...
        o.tab.animate(
          { marginTop: -tabHeight },
          {
            ...
            complete: function () {
              o.tab
                .css({ display: "none", marginTop: "", width: "" })
